...
import { calculateRewardRedemption, getRewardSummary, reserveRewardRedemption, restoreRewardRedemption } from "@/lib/rewards";
...
function getSiteUrl() {
  ...
  try { const url = new URL(raw); return url.protocol === "http:" || url.protocol === "https:" ? url.origin : FALLBACK_SITE; } catch { return FALLBACK_SITE; }
}
...
export async function startCheckout(_prev: CheckoutState, formData: FormData): Promise<CheckoutState> {
  ...
  const parsed = checkoutSchema.safeParse({
    email: formData.get("email"), shippingRateId: formData.get("shippingRateId"), notes: formData.get("notes") ?? "", saveAddress: formData.get("saveAddress") === "on",
    address: { fullName: formData.get("fullName"), line1: formData.get("line1"), line2: formData.get("line2"), city: formData.get("city"), region: formData.get("region"), postalCode: formData.get("postalCode"), country: formData.get("country"), phone: formData.get("phone") },
  });
  ...
  const requestedRewardPoints = Math.max(0, parseInt(String(formData.get("rewardPoints") ?? "0"), 10) || 0);
  ...
  if (!user && requestedRewardPoints > 0) return { error: "Sign in to use Mama Rewards." };
  ...
  let promoDiscountCents = 0, freeShipping = false, discountCode: string | null = null;
  if (cart.discount) {
    ...
    promoDiscountCents = v.discountCents; freeShipping = v.freeShipping; discountCode = v.discount.code;
  }

  const eligibleSubtotalCents = Math.max(0, totals.subtotalCents - promoDiscountCents);
  let reward = { points: 0, percent: 0, discountCents: 0 };
  if (requestedRewardPoints > 0 && user) {
    const summary = await getRewardSummary(user.id);
    reward = await calculateRewardRedemption(requestedRewardPoints, summary.points, eligibleSubtotalCents);
    if (reward.points <= 0) return { error: "These Mama Rewards points cannot be applied. Check your balance and redemption minimum." };
  }

  const discountCents = promoDiscountCents + reward.discountCents;
  // Reward redemption never removes an already-earned free-shipping threshold.
  const shippingCents = freeShipping ? 0 : computeShippingCents(rate, eligibleSubtotalCents, currency);
  const totalCents = Math.max(0, totals.subtotalCents - discountCents + shippingCents);
  ...
  try {
    orderId = await db.$transaction(async (tx) => {
      const order = await tx.order.create({
        data: {
          ...
          items: { create: items.map((i) => ({ variantId: i.variantId, productName: i.variant.product.name, variantName: i.variant.name, sku: i.variant.sku, options: i.variant.options ?? undefined, imageUrl: i.variant.image?.url ?? i.variant.product.images[0]?.url ?? null, unitCents: i.variant.priceCents, quantity: i.quantity, totalCents: i.variant.priceCents * i.quantity })) },
          payment: { create: { provider: "STRIPE", amountCents: totalCents, currency } },
          history: { create: { status: "PENDING_PAYMENT", actor: "system", note: reward.points > 0 ? `Order created · ${reward.points} reward points reserved` : "Order created" } },
        },
      });
      if (reward.points > 0 && user) {
        const reserved = await reserveRewardRedemption(tx, order.id, user.id, reward.points);
        if (!reserved) throw new Error("REWARD_POINTS_UNAVAILABLE");
      }
    ...
    if ((err as Error).message === "REWARD_POINTS_UNAVAILABLE") return { error: "Your Mama Rewards balance changed. Refresh checkout and try again." };
  ...
  try {
    ...
    const result = await provider.createCheckout({
      ...
      discountCents, discountLabel: [discountCode, reward.points > 0 ? `Mama Rewards ${reward.points} pts` : null].filter(Boolean).join(" + ") || undefined,
      address, locale, expiresAt, automaticTax: payments.stripeTaxEnabled && process.env.STRIPE_TAX_ENABLED !== "false",
      successUrl: `${SITE}/checkout/success?session_id={CHECKOUT_SESSION_ID}`, cancelUrl: `${SITE}/cart?cancelled=1`,
    });
    ...
    try {
      ...
      await restoreRewardRedemption(orderId);
    } catch (rollbackErr) { await logger.error("checkout.rollback_failed", rollbackErr, { orderId }); }
