...
export async function signUp(_prev: AuthState, formData: FormData): Promise<AuthState> {
  ...
  const { data, error } = await supabase.auth.signUp({
    ...parsed.data,
    options: { data: { first_name: firstName, last_name: lastName }, emailRedirectTo: `${SITE}/auth/callback?next=/account` },
  });
  if (error) return { error: error.message };
  if (data.user?.email) void sendWelcome(data.user.email, firstName).catch((e) => logger.error("welcome.failed", e));
