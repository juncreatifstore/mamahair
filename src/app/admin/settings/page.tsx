...
const SECTIONS: { key: SettingSection; title: string; help?: string }[] = [
  ...
  { key: "payments", title: "Payments", help: "Stripe is live. Mercado Pago (OXXO, SPEI) and PayPal are prepared in lib/payments and can be switched on once implemented." },
  ...
  { key: "policies", title: "Policies", help: "Shipping, returns, privacy and terms. Shown at /pages/shipping, /pages/returns, /pages/privacy, /pages/terms unless a page with that slug exists." },
];

const LABELS: Record<string, string> = { companyName: "Company name", storeName: "Store name (short)", tagline: "Tagline", announcement: "Announcement bar text", announcementEnabled: "Show announcement bar", logoUrl: "Main logo URL", logoLightUrl: "Light logo URL (dark backgrounds)", logoDarkUrl: "Dark logo URL", faviconUrl: "Favicon URL", primaryColor: "Primary color", accentColor: "Accent color", email: "Email", phone: "Phone", whatsapp: "WhatsApp number (with country code)", addressLine: "Address", city: "City", region: "State / region", postalCode: "Postal code", country: "Country (ISO)", instagram: "Instagram", facebook: "Facebook", tiktok: "TikTok", youtube: "YouTube", pinterest: "Pinterest", defaultCurrency: "Default currency", enabledCurrencies: "Enabled currencies", enabledCountries: "Countries you ship to", lowStockThreshold: "Low stock threshold", reservationMinutes: "Stock reservation (minutes, min 30)", freeShippingBannerCents: "Free shipping threshold shown in banners (cents)", allowGuestCheckout: "Allow guest checkout", stripeEnabled: "Stripe enabled", stripeTaxEnabled: "Stripe Tax (automatic tax)", applePayGooglePay: "Apple Pay / Google Pay (Stripe Checkout)", mercadoPagoEnabled: "Mercado Pago (Mexico)", paypalEnabled: "PayPal", originCountry: "Ship-from country", handlingDays: "Handling days", carrierIntegration: "Carrier integration (none / shippo / easypost)", fromName: "From name", fromEmail: "From email", replyTo: "Reply-to", footerText: "Email footer text", defaultTitle: "Default title", defaultDescription: "Default description", ogImageUrl: "OpenGraph image URL", twitterHandle: "Twitter/X handle", defaultLocale: "Default language", enabledLocales: "Enabled languages", defaultCountry: "Default country", shipping: "Shipping policy", returns: "Return policy", privacy: "Privacy policy", terms: "Terms & conditions" };
      ...
      <Card title={current.title} className="mt-4 max-w-3xl">
        ...
        <form action={async (fd) => { "use server"; await saveSettingsSection(current.key, fd); }} className="space-y-4">
          {Object.entries(DEFAULT_SETTINGS[current.key]).map(([k, def]) => {
            ...
            if (typeof def === "number") return <Field key={k} label={label}><Input name={k} type="number" defaultValue={String(v ?? def)} /></Field>;
            ...
            return <Field key={k} label={label}><Input name={k} defaultValue={String(v ?? "")} type={k.toLowerCase().includes("color") ? "text" : "text"} /></Field>;
          })}
